...
>>>>>>> 23014cb9b6ce4789b8f9e8b074ec7b2dba92e8af
    std::cout << YELLOW << "Client::parseMultipartRequest starting: boundary = "
...
>>>>>>> 23014cb9b6ce4789b8f9e8b074ec7b2dba92e8af
  ...
  void Client::setupBodyParsing(void) {
    ...
    if (itLength != _headers.end()) {
      ...
      _chunkFile += "id" + t.str();
      _chunkFd =
          open(_chunkFile.c_str(), O_CREAT | O_RDWR | O_TRUNC | O_CLOEXEC);
      if (_chunkFd == -1) {
        _statusCode = 500;
        return;
      }
  ...
  void Client::checkPathInfo(void) {
    ...
    std::cout << YELLOW << "_sPathinfo = " << _sPathInfo
              << "; _sUri = " << _sUri << RESET << std::endl;
  }
  ...
  void Client::parseRequest(std::string & buffer) {
    ...
    if (request.size() < 2) {
      ...
      std::cout << RED
                << "changin statusCode because if (request.size() < 2) to "
                << _statusCode << RESET << std::endl;
